...
outputs, _ = run_net(inputs = xs, max_predictions = cfg.max_predictions, num_classes = cfg.num_classes, is_train = cfg.is_train)
loss_op = utility.calc_loss(logits = ys, predictions = outputs)
train_op = tf.train.GradientDescentOptimizer(learning_rate = learning_rate).minimize(loss_op)
accuracy_op = utility.calc_accuracy(logits = ys, predictions = outputs)
